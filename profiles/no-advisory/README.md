...
- Reason no IDD-managed advisory reviewer is used: this solo-maintainer
  repository relies on CI and any future repository-level protections.
- Branch protection rule: no required checks or reviews were configured on
  `main` at installation time; future protection remains authoritative.
- Human review rule outside IDD, if any: none configured; human feedback is
  still collected and can block the normal review gates.
- Review-thread resolution profile: `fast-agent-resolve`, subject to branch
  protection conversation rules.
- Merge policy: `fully_autonomous_merge`.
- Verification PR or dry-run reference: initial direct-import verification;
  subsequent changes use Issue -> claim -> PR -> CI -> autonomous merge.
