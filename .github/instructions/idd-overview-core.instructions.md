...
| Name | Commands |
| --- | --- |
| **fix-validate** | `npm run format && npm run check` |
| **pre-push-validate** | `npm run typecheck && npm run check && npm test` |
| **post-fix-validate** | `npm run format && npm run typecheck && npm run check && npm test` |
| **install-deps** | `npm ci` |
...
Review-policy-specific behavior lives in the selected phase files. This
repository uses `no-advisory`, so `idd-advisory-wait.instructions.md` is an
explicit no-op and E14/F2/F3 must not request or wait for advisory reviews.
Do not reintroduce Copilot wait logic into caller files without first changing
`.github/idd/config.json` and the repository-local policy record.
